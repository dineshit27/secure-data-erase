...
const Auth = () => {
  ...
  const { signIn, signUp, signInWithGithub, signInWithGoogle, resetPassword, continueAsGuest, user } = useAuth();
              ...
              <motion.div key="login" initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} transition={{ duration: 0.25 }}>
                ...
                <div className="grid grid-cols-2 gap-3 mb-6">
                  <button
                    ...
                    className="h-12 flex items-center justify-center gap-2.5 rounded-lg bg-surface-2 border border-border hover:border-primary/30 font-mono text-sm text-text-primary transition-all hover:bg-surface"
                  >
                    <GoogleIcon />
                    Google
                  </button>
