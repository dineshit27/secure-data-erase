...
const BrowserCleaner = () => {
  ...
  const startWipe = async () => {
    ...
    logClientProgress({
      toolId: "browser-cleaner",
      stage: "run_start",
      endpoint: "client:browser_storage_wipe",
      requestId,
      details: { selectedTargets },
    });
    ...
    for (const targetId of selectedTargets) {
      ...
      try {
        ...
        logClientProgress({
          ...
          requestId,
          status: "error",
          details: { target: targetId, error: String(err) },
        });
        continue;
      }
    ...
    logClientRun({
      ...
      details: {
        ...
        remaining: {
          localStorageKeys: after.localStorageKeys,
          sessionStorageKeys: after.sessionStorageKeys,
          indexedDB: after.indexedDBNames.length,
          caches: after.cacheNames.length,
        ...
        <>
          ...
          <div className="space-y-2 mb-8">
            {targets.map((t) => (
              <label
                key={t.id}
                className={`flex items-center justify-between p-3 rounded-lg bg-surface border cursor-pointer transition-all ${t.available ? "border-border hover:border-border/60" : "border-border/30 opacity-50"
                  }`}
                data-interactive
              >
